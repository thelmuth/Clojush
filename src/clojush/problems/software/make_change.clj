...
(ns clojush.problems.software.make-change
...
(defn make-make-change-error-function-from-cases
  ...
  (fn the-actual-make-change-error-function
    ([individual]
      (the-actual-make-change-error-function individual :train))
    ([individual data-cases] ;; data-cases should be :train or :test
     (the-actual-make-change-error-function individual data-cases false))
...
(defn get-make-change-train-and-test
  "Returns the train and test cases."
  [data-domains]
  (map make-change-test-cases
          (test-and-train-data-from-domains data-domains)))

; Define train and test cases
(def make-change-train-and-test-cases
  (get-make-change-train-and-test make-change-data-domains))

(defn make-change-initial-report
  [argmap]
  (println "Train and test cases:")
  (doseq [[i case] (map vector (range) (first make-change-train-and-test-cases))]
    (println (format "Train Case: %3d | Input/Output: %s" i (str case))))
  (doseq [[i case] (map vector (range) (second make-change-train-and-test-cases))]
    (println (format "Test Case: %3d | Input/Output: %s" i (str case))))
  (println ";;******************************"))

(defn make-change-report
  "Custom generational report."
  [best population generation error-function report-simplifications]
  (let [best-test-errors (:test-errors (error-function best :test))
        best-total-test-error (apply +' best-test-errors)]
    (println ";;******************************")
    (printf ";; -*- Make Change problem report - generation %s\n" generation)(flush)
...
; Define the argmap
(def argmap
  {:error-function (make-make-change-error-function-from-cases (first make-change-train-and-test-cases)
                                                             (second make-change-train-and-test-cases))
   :atom-generators make-change-atom-generators
   :max-points 1600
   ...
   :problem-specific-report make-change-report
   :problem-specific-initial-report make-change-initial-report
