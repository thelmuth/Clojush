...
(def cut-vector-atom-generators
  (concat (list
            ;;; end constants
            (fn [] (cut-vector-input (inc (lrand-int 50)))) ;Vector ERC
            ;;; end ERCs
...
(def cut-vector-data-domains
  [[(list [0] [100] [-100] [1000] [-1000]) 5 0] ;; Length-1 vectors
   ...
   [(fn [] (cut-vector-input 50)) 5 50] ;; Random Length-50 vectors
   [(fn [] (cut-vector-input (inc (lrand-int 50)))) 95 1000] ;; Random length, random ints
   ])
